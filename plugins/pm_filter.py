...
from database.connections_mdb import (
    add_connection, active_connection,
    all_connections, if_active, make_active,
    make_inactive, delete_connection,
)
import random
...
from Streaming.util.file_properties import get_name, get_hash
...
async def admin_commands(client, query):
    ...
    await client.edit_message_media(
        ...
        media=InputMediaAnimation(
            #media="https://media4.giphy.com/media/v1.Y2lkPTc5MGI3NjExb2trajF5eThxN2h5d2kzaXl6aTZ5MWhpbWEya2doNjBnbGtvMnZiYiZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9Zw/PnoQLaOLYp93ebw382/giphy.gif",
...
async def advantage_spoll_choker(bot, query):
    ...
    if files:
        ...
        k = await query.message.edit(
            ...
            reply_markup=InlineKeyboardMarkup(
                [[InlineKeyboardButton("📨 Request to Admin", url="https://t.me/Contact_Vicky_via_bot")]]
            )
        )
...
async def cb_handler(client: Client, query: CallbackQuery):
    if query.data == "close_data":
        ...
        buttons = [
            ...
            [
                ...
                InlineKeyboardButton("💎 Gᴇᴛ Pʀᴇᴍɪᴍᴜᴍ - Nᴏ Aᴅꜱ, Nᴏ Vᴇʀɪғɪᴄᴀᴛɪᴏɴ💎", callback_data="vickypremium"),
            ],
            [
    ...
    #        )
        ...
        await client.edit_message_media(
            ...
            message_id=query.message.id,
            media=InputMediaAnimation(
                media=SUBSCRIPTION,  # Replace with your GIF URL or file_id
        ...
        buttons = [
            [
                InlineKeyboardButton(
                    "🍁 ᴄʜᴇᴄᴋ ᴀʟʟ ᴘʟᴀɴꜱ & ᴘʀɪᴄᴇꜱ 🍁", callback_data="checkplans"
                )
            ],
        ...
        await client.edit_message_media(
            ...
            message_id=query.message.id,
            media=InputMediaAnimation(
                media=SUBSCRIPTION,  # this should be a GIF/MP4 URL or a Telegram file_id
        ...
        # # Optional sticker intro
        # m = await query.message.reply_sticker(
        #     "CAACAgUAAx0CZz_GMwACMBdnXZA4SejgJ6a_0TrNzOfn9ImI_QACNwsAArT4iFVaZPJf8ldVVh4E"
        # )
        # await m.delete()

        # Send animated GIF or MP4
        await query.message.reply_animation(
            animation=SUBSCRIPTION,  # must be a .gif/.mp4 URL or Telegram file ID
            caption=script.PREPLANSS_TXT.format(query.from_user.mention),
            reply_markup=reply_markup,
            parse_mode=enums.ParseMode.HTML,
        )

    elif query.data == "checkplans":
        buttons = [
            [
                InlineKeyboardButton(
                    "☆📸 ꜱᴇɴᴅ ꜱᴄʀᴇᴇɴꜱʜᴏᴛ 📸☆",
        ...
        try:
            ...
            # Generate a mention (HTML format)
            mention = f"<a href='tg://user?id={query.from_user.id}'>{query.from_user.first_name}</a>"

            # Send a new photo message with caption and buttons
            await client.send_photo(
                chat_id=query.message.chat.id,
                photo=random.choice(PAYPICS),  # Random image from your list
                caption=script.FREE_TXT.format(mention),
                parse_mode=enums.ParseMode.HTML,
                reply_markup=reply_markup,
            )
        except Exception as e:
            print("❌ Error sending new photo in 'checkplans':", e)
        ...
        buttons = [
            [
                InlineKeyboardButton(
                    "📲 ᴄᴏɴᴛᴀᴄᴛ ᴛᴏ ᴏᴡɴᴇʀ", url=f"https://telegram.me/{CONTACT_OWNER}"
                )
            ],
        ...
        await query.message.edit_text(
            ...
            script.ABOUT_TEXT.format(temp.B_NAME),
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(
        ...
        btn = [
            [
                InlineKeyboardButton(
                    "📲 ᴄᴏɴᴛᴀᴄᴛ ᴛᴏ ᴏᴡɴᴇʀ ", url=CONTACT_OWNER
                )
            ],
            [InlineKeyboardButton("⇋ ʙᴀᴄᴋ ⇋", callback_data="about")],
        ]
...
async def auto_filter(client, msg, spoll=False, pm_mode=False):
    ...
    del_msg = (
        f"\n\n<b>⚠️  m<code>{get_readable_time(DELETE_TIME)}</code> ᴛᴏ ᴀᴠᴏɪᴅ ᴄᴏᴘʏʀɪɢʜᴛ ɪssᴜᴇs</b>"
        if settings["auto_delete"]
        else ""
    )
    links = ""
